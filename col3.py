...
class Simulation:
    ...
    def place_particle(self, rad, styles):
        ...
        x = rad + (100 - 2*rad) * np.random.random()
        y = rad + (3*rad - 2*rad) * np.random.random()
        # Choose a random velocity (within some reasonable range of
        # values) for the Particle.
        # vr = np.sqrt(-2.00 * np.log(np.random.random())) 
        # vphi = 2*np.pi * np.random.random()
        vr1 =tx * np.sqrt(np.random.random()) + 0.05
        vr2 =ty * np.sqrt(np.random.random()) + 0.05
        vphi = 2*np.pi * np.random.random()
        vx, vy = vr1 * np.cos(vphi), vr2 * np.sin(vphi)
        
        # vx=np.sqrt(self.n*1.0/vx**2)*vx 
        # vy=np.sqrt(self.n*5.0/vy**2)*vy 
    ...
    def change_velocities(self, p1, p2):
        ...
        #! Inelastic collision
        u1 = v1 - (1+0.999)*m2 / M * np.dot(v1-v2, r1-r2) / d * (r1 - r2)
        u2 = v2 - (1+0.999)*m1 / M * np.dot(v2-v1, r2-r1) / d * (r2 - r1)
    ...
    def handle_boundary_collisions(self, p):
        ...
        if p.y - p.radius < 0:
            p.y = p.radius
            p.vy = -p.vy +2*vp
        if p.y + p.radius > 3*p.radius:
            p.y = 3*p.radius-p.radius
            p.vy = -p.vy 
    ...
    def setup_animation(self):
        self.fig, self.ax = plt.subplots(figsize=(25,6))
        for s in ['top','bottom','left','right']:
            self.ax.spines[s].set_linewidth(2)
        self.ax.set_aspect('equal','box')
        self.ax.set_xlim(0, 200)
        self.ax.set_ylim(0, 3*radii)
        self.ax.xaxis.set_ticks([])
        self.ax.yaxis.set_ticks([])
       
        # self.ax.set_aspect(10,'equal')
        #     # changing x scale by own
