...
class Simulation:
    ...
    def __init__(self, n, radius=0.01, styles=None):
        ...
        self.rho = 0.03
        self.h= 3.0*radius 
        self.epsilon =( self.h-2*radius )/radius
        self.longy = n/(self.rho * (self.h - self.epsilon*radius))
    ...
    def handle_boundary_collisions(self, p):
        ...
        if p.x - p.radius < 0:
            p.x = p.radius
            p.vx = -p.vx
        if p.x + p.radius > self.h:
            p.x = self.h-p.radius
            p.vx = -p.vx
        if p.y - p.radius < 0:
            p.y = p.radius
            p.vy = -p.vy
        if p.y + p.radius > self.h:
            p.y = self.h-p.radius
            p.vy = -p.vy
